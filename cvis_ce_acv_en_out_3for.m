function cvis_ce_acv_en_out_3for()
    ...
    n0_CV_EN = fix(n0_CV/(nbatches*outer));
    n1_CV_EN = fix(n1_CV/(nbatches*outer));
    ...
    for j = 1:outer
        ...
        astar = -cov_MC_Q0Q1_CV_EN(1,2)/cov_MC_Q0Q1_CV_EN(2,2);
        CV_EN(j) = mean(wQ0s_CV_EN) + astar*(mean(wQ1s_CV_EN)-prob1);
    end
    
    v_CV_EN = var(CV_EN)/outer;
    disp('v_CV_EN/v0_MC');
    disp(v_CV_EN/v0_MC);
    ...
    n0_IS_EN = fix(n0_IS/(nbatches*outer));
    n1_IS_EN = fix(n1_IS/(nbatches*outer));
    m1_IS_EN = fix(m1_IS/(nbatches*outer));
    ...
    for j = 1:outer
        ...
        IS_EN(j) = mean(wQ0s_IS_EN) + astar*(mean(wQ1s_IS_EN)-mean(mu1_IS));
    end
    
    v_IS_EN = var(IS_EN)/outer;
    disp('v_IS_EN/v0_MC');
    disp(v_IS_EN/v0_MC);
